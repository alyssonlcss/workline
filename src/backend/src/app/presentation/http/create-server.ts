...
export async function createServer() {
  ...
  server.post('/api/scanner/data-download', async (request, reply) => {
    ...
    // Dispara a carga do Openview em Background
    try {
      const baseFilter = payload.selectedFilters?.find(f => f.title === 'Base' || f.title === 'Polo');
      if (baseFilter && baseFilter.selectedValues.length > 0 && incidenceService) {
        const baseName = baseFilter.selectedValues[0].toUpperCase();
        const baseToPolo: Record<string, string> = {
          'ATLÂNTICO': 'ATLANTICO', 'ATLANTICO': 'ATLANTICO', 'CENTRO-NORTE': 'DECEN', 'NORTE': 'DNORT',
          'CENTRO-SUL': 'DECES', 'FORTALEZA': 'DEMEF', 'LESTE': 'DLEST', 'METROPOLITANA': 'DEMEM', 'SUL': 'DSUL'
        };
        const polo = baseToPolo[baseName] || baseName;

        let months: string[] = [];
        if (payload.periodSelection?.month) {
           months = Array.isArray(payload.periodSelection.month) ? payload.periodSelection.month : [payload.periodSelection.month];
        } else {
           const monthFilter = payload.selectedFilters?.find(f => f.title.toLowerCase().includes('mês') || f.title.toLowerCase().includes('mes'));
           if (monthFilter) months = monthFilter.selectedValues;
        }

        let years: string[] = [];
        if (payload.periodSelection?.year) {
           years = Array.isArray(payload.periodSelection.year) ? payload.periodSelection.year : [payload.periodSelection.year];
        } else {
           const yearFilter = payload.selectedFilters?.find(f => f.title.toLowerCase().includes('ano'));
      ...
      console.error('Failed to trigger openview prefetch', e);
    }
  ...
  if (incidenceEnabled) {
    ...
    startScannerJob.setIncidenceService(incidenceService);
  ...
  server.get('/api/incidence/get-incidencias', async (request, reply) => {
    if (!incidenceService) {
