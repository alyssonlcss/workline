      ...
      for (const flag of ev.flags) {
        switch (flag) {
          ...
          case 'temp_prep_alto': {
            ...
            alertTexts[flag] = `o técnico levou ${tempPrepMin} min entre ${subject} — ${pct}% acima do limite de ${getTempPrepLimit()} min. Esse tempo representa espera antes de se deslocar para o próximo atendimento.`;
            break;
          }
          case 'sem_os_alto': {
            ...
            alertTexts[flag] = `${minVal} min sem OS registrada — acima do limite de ${getSemOsLimit()} min. Esse tempo representa intervalos ociosos em que o técnico não estava atendendo nem a caminho de um chamado.`;
            break;
          }
          case 'inicio_jornada_alto': {
            ...
            if (ev.flags.includes('login_atrasado' as any)) {
              ...
              alertTexts[flag] = `${nfBr(totalMin)} min do Início Calendário até o primeiro despacho — acima do limite de ${getSemOsLimit()} min. Esse tempo representa espera ociosa no início da jornada.`;
            }
            break;
          }
          case 'desloc_intervalo_alto': {
            const detail = ev.sem_os_details?.find(d => d.type === 'intervalo_deslocamento');
            alertTexts[flag] = `${Math.round(detail?.min ?? 0)} min de Deslocamento de Intervalo — acima do limite de ${getSemOsLimit()} min. Esse tempo ocioso ocorreu antes de iniciar o deslocamento ou durante a pausa.`;
            break;
          }
          ...
          case 'login_atrasado': {
            ...
            const linTs = parseDt(logInVal || '');
            const delayMin = icalTs > 0 && linTs > 0 ? Math.round((linTs - icalTs) / 60000) : getLoginAtrasadoLimit();
            alertTexts[flag] = `a equipe registrou acesso ao sistema (Log In) com ${delayMin} min de atraso em relação ao Início Calendário (acima do limite de ${getLoginAtrasadoLimit()} min). O atraso compromete diretamente o tempo de deslocamento.`;
            break;
          }
          ...
          case 'primeiro_desloc_alto': {
            ...
            alertTexts[flag] = `o tempo desde o Início Calendário até o primeiro registro de 'A Caminho' foi de ${nfBr(val)} min — ${pct}% acima do limite de ${getPrimeiroDeslocLimit()} min. Esse tempo reflete o tempo total ocioso no início da jornada antes do primeiro deslocamento.`;
            break;
          }
        }
      }
        ...
        switch (flag) {
          case 'despacho_tardio':
            if (ev.flags.includes('login_atrasado')) {
              ...
              alertTexts[flag] = `a equipe recebeu a primeira OS com ${nfBr(ev.despacho_apos_inicio_min)} min de atraso em relação ao início da jornada - acima do limite de ${getSemOsLimit()} min. Esse atraso na fila inicial de distribuição reduz o tempo disponível para atendimentos no dia.`;
            }
            break;
          case 'login_atrasado':
