...
// 1. Inject enrichedIncidenceData
if (!content.includes('enrichedIncidenceData')) {
...
// 2. Inject fetchIncidenceBatch and triggerIncidenceBatchFetch
if (!content.includes('fetchIncidenceBatch')) {
  ...
  // Call it inside updateReportDataAndDates
  content = content.replace(
...
// 3. Inject lazy load inside toggleDateExpanded
const toggleExpandedReplacement = `protected toggleDateExpanded(kpiKey: string, team: string, dateRef: string): void {
...
if (regex.test(content)) {
  content = content.replace(regex, toggleExpandedReplacement);
}

// 4. Also make sure getIncidenceForOrder and getIncidenceTags use padStart
content = content.replace(/String\((nrOrdem|enriched\.incidenceNumber|order\.nr_ordem)\)\.replace\(\/\^0\+\/, ''\)/g, "String($1).padStart(10, '0')");
