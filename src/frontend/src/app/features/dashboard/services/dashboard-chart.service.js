...
let DashboardChartService = class DashboardChartService {
    constructor() {
        this.CHART_COLORS = [
    ...
    getDeviationsForDay(deviations, day) {
        ...
        return deviations.filter((d) => {
            ...
            if (d.dateRef.startsWith(day + '/'))
                return true;
            // inverse: dateRef is dd/mm and dayLabel is dd/mm/yyyy
            if (day.startsWith(d.dateRef + '/'))
                return true;
            return false;
        });
    }
    getTeamFlagSummary(kpi, team, report) {
        ...
        const semOsSubData = new Map();
        const bump = (map, key, min) => {
                ...
                if (entry) {
                    for (const order of entry.flaggedOrders) {
                        for (const f of order.flags) {
                            let min = 0;
                            if (f === 'tr_excede_hd')
                                min = order.tr_ordem_min;
                            else if (f === 'tl_excede_hd')
                                min = order.tl_ordem_min;
                            else if (f === 'temp_prep_alto')
                                min = order.temp_prep_os_min ?? 0;
                            else if (f === 'sem_os_alto')
                                min = order.sem_os_total_min ?? 0;
                            bump(flagData, f, min);
            ...
            case 'Eficiência': {
                ...
                if (entry) {
                    for (const order of entry.flaggedOrders) {
                        for (const f of order.flags) {
                            let min = 0;
                            if (f === 'tr_excede_hd' || f === 'tr_muito_baixo')
                                min = order.tr_ordem_min;
                            else if (f === 'deslocamento_curto')
                                min = order.tl_ordem_min;
                            bump(flagData, f, min);
                        }
                    }
                ...
                if (entry) {
                    for (const order of entry.flaggedOrders) {
                        for (const f of order.flags) {
                            let min = 0;
                            if (f === 'tme_muito_alto')
                                min = order.tme_imp_min;
                            else if (f === 'sem_deslocamento')
                                min = order.tl_ordem_min;
                            else if (f === 'sem_execucao')
                                min = order.tr_ordem_min;
                            bump(flagData, f, min);
                        }
                    }
            ...
            case '1º Desloc.': {
                ...
                if (entry) {
                    for (const day of entry.flaggedDays) {
                        for (const f of day.flags) {
                            let min = 0;
                            if (f === 'desloc_lento' || f === 'desloc_muito_lento')
                                min = day.primeiro_desloc_min ?? 0;
                            else if (f === 'despacho_tardio')
                                min = day.despacho_apos_inicio_min ?? 0;
                            bump(flagData, f, min);
                        }
                    }
                }
                ...
                if (entry) {
                    for (const d of entry.flaggedDays) {
                        ...
                        for (const f of d.flags) {
                            let min = 0;
                            if (f === 'desloc_lento' || f === 'desloc_muito_lento')
                                min = d.primeiro_desloc_min ?? 0;
                            else if (f === 'despacho_tardio')
                                min = d.despacho_apos_inicio_min ?? 0;
                            bump(flagData, f, min);
                        }
                    }
                }
    ...
    analyticChartData(kpi, report, comparePartner = false) {
        ...
        const fmt = (v) => (v % 1 === 0 ? String(Math.round(v)) : v.toFixed(1));
        const aboveFn = (v) => kpi.direction === 'higher-is-better' ? v >= kpi.metaTarget : v <= kpi.metaTarget;
        ...
        const buildDevMap = (team) => {
            const map = new Map();
            const add = (dateRef, flags, detail) => {
                const key = dateRef;
                const existing = map.get(key);
                if (existing) {
                    for (const f of flags)
                        if (!existing.flags.includes(f))
            ...
            const specialAnalysisMap = {};
            // Map kpi.kpi → specialAnalysis field
            if (kpi.evidenceAnalysis)
                specialAnalysisMap['Eficiência'] = kpi.evidenceAnalysis;
            if (kpi.tmeImpAnalysis)
                specialAnalysisMap['TME IMP'] = kpi.tmeImpAnalysis;
            if (kpi.primeiroLoginAnalysis)
                specialAnalysisMap['1º Login'] = kpi.primeiroLoginAnalysis;
            if (kpi.primeiroDeslocAnalysis)
                specialAnalysisMap['1º Desloc.'] = kpi.primeiroDeslocAnalysis;
            if (kpi.retornoBaseAnalysis)
                specialAnalysisMap['Retorno Base'] = kpi.retornoBaseAnalysis;
            // OS Dia and Utilização live in specialAnalysis (not attached to the kpi object)
            if (report) {
                if (kpi.kpi === 'OS Dia')
                    specialAnalysisMap['OS Dia'] = report.specialAnalysis.osDiaAnalysis;
                if (kpi.kpi === 'Utilização')
                    specialAnalysisMap['Utilização'] = report.specialAnalysis.utilizacaoAnalysis;
            }
            ...
            if (teamEntry) {
                ...
                (teamEntry.flaggedDays ?? []).forEach((d) => {
                    if (d.date_ref)
                        add(d.date_ref, d.flags.map(flagLabel));
                });
            }
            return map;
        };
        ...
        const toX = (i) => padLeft + innerPadX + (D > 1 ? (i / (D - 1)) * (chartW - 2 * innerPadX) : (chartW - 2 * innerPadX) / 2);
        // ── Build per-team daily lookup (for non-flat team lines) ─────────────────
        const perTeamDailyMap = new Map();
        ...
        if (kpi.dailyTrendByBase && kpi.dailyTrendByBase.length > 0) {
            for (const tb of kpi.dailyTrendByBase) {
                if (isBaseAll && tb.base === 'Média Global')
                    continue;
                if (!isBaseAll && !bases.includes(tb.base))
                    continue;
                if (wantsCombined && tb.teamType !== 'All')
                    continue;
                if (wantsContrast && tb.teamType !== 'propria' && tb.teamType !== 'parceira' && tb.teamType !== 'Própria' && tb.teamType !== 'Parceira')
                    continue;
                if (!wantsCombined && !wantsContrast) {
                    if (wantsPropria && !wantsParceira && tb.teamType !== 'propria' && tb.teamType !== 'Própria')
                        continue;
                    if (wantsParceira && !wantsPropria && tb.teamType !== 'parceira' && tb.teamType !== 'Parceira')
                        continue;
                    if (wantsPropria && wantsParceira && tb.teamType !== 'propria' && tb.teamType !== 'Própria' && tb.teamType !== 'parceira' && tb.teamType !== 'Parceira')
                        continue;
                }
        ...
        const toY = (v) => padTop + chartH * (1 - (v - minVal) / (maxVal - minVal));
        ...
        const lines = kpi.scores.map((score, si) => {
            ...
            const deviations = [...devMap.values()].sort((a, b) => parseDay(a.dateRef) - parseDay(b.dateRef));
            return {
                team: score.team,
                color,
        ...
        if (hasDailyTrend) {
            ...
            if (kpi.dailyTrendByBase && kpi.dailyTrendByBase.length > 0) {
                for (const tb of kpi.dailyTrendByBase) {
                    if (isBaseAll) {
                        if (tb.base === 'Média Global')
                            continue;
                    }
                    else {
                        if (!bases.includes(tb.base))
                            continue;
                    }
                    if (wantsCombined) {
                        if (tb.teamType !== 'All')
                        ...
                        if (tb.teamType !== 'propria' && tb.teamType !== 'parceira' && tb.teamType !== 'Própria' && tb.teamType !== 'Parceira')
                            continue;
                    }
                    else {
                        if (wantsPropria && !wantsParceira && tb.teamType !== 'propria' && tb.teamType !== 'Própria')
        ...
        const dayPointsMap = new Map();
        for (const tl of trendLines) {
            for (const pt of tl.points) {
                pt.cx = pt.x;
                pt.cy = pt.y;
                if (!dayPointsMap.has(pt.label))
                    dayPointsMap.set(pt.label, []);
        ...
        for (const l of lines) {
            for (const pt of l.points) {
                pt.cx = pt.x;
                pt.cy = pt.y;
                if (!dayPointsMap.has(pt.dayLabel))
                    dayPointsMap.set(pt.dayLabel, []);
