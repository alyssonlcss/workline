var DashboardPdfService_1;
import { __decorate } from "tslib";
// Copyright (c) 2026 Alysson Pinheiro. Todos os direitos reservados.
// Software proprietário e confidencial. Uso não autorizado é proibido.
import { Injectable } from '@angular/core';
import { buildTimelineSegments, extractTime, parseDt, tlFlexGrow } from '../../../shared/utils/timeline-segment.utils';
// eslint-disable-next-line @typescript-eslint/no-require-imports
const pdfMake = require('pdfmake/build/pdfmake');
// eslint-disable-next-line @typescript-eslint/no-require-imports
const pdfFonts = require('pdfmake/build/vfs_fonts');
pdfMake.vfs = pdfFonts.pdfMake?.vfs ?? pdfFonts.vfs;
let DashboardPdfService = class DashboardPdfService {
    static { DashboardPdfService_1 = this; }
    static { this.TIMELINE_IDLE_LABELS = new Set([
    ...
    buildTimelinePdfBlock(ev, hidePartida = false, trimToACaminho = false) {
        const segs = buildTimelineSegments(ev, hidePartida, trimToACaminho);
        if (!segs.length)
            return null;
        const IDLE = DashboardPdfService_1.TIMELINE_IDLE_LABELS;
        const isRepairAlarm = (s) => (s.label === 'Reparo' || s.label === 'Log In') && (s.flags?.length ?? 0) > 0;
        const isDeslocAlarm = (s) => s.label === 'Deslocamento p/OS' && (s.flags?.length ?? 0) > 0;
        const isIdleLabel = (s) => IDLE.has(s.label) || s.label.startsWith('1º Despacho:');
        const isRetornoBaseAlarm = (s) => s.label === 'Retorno a base' && (s.flags?.length ?? 0) > 0;
        const getFill = (s) => s.isInterval ? '#fde68a' : isRepairAlarm(s) || isDeslocAlarm(s) || isRetornoBaseAlarm(s) ? '#fca5a5' :
            s.label === 'Deslocamento p/OS' ? '#dbeafe' :
                isIdleLabel(s) ? ((s.flags?.length ?? 0) > 0 ? '#fca5a5' : '#fee2e2') : '#dbeafe';
        const getTxtColor = (s) => s.isInterval ? '#78350f' : (isRepairAlarm(s) || isDeslocAlarm(s) || isRetornoBaseAlarm(s) || (isIdleLabel(s) && s.label !== 'Deslocamento p/OS' && (s.flags?.length ?? 0) > 0)) ? '#7f1d1d' : (isIdleLabel(s) && s.label !== 'Deslocamento p/OS') ? '#7f1d1d' : '#1e3a8a';
        ...
        const barRow = segs.map((s) => ({
            stack: [
                { text: s.label, fontSize: 5.5, bold: true, color: getTxtColor(s), alignment: 'center' },
                { text: `${s.overrideDuration ?? `${s.durationMin}min`}${s.subtitle ? ` | ${s.subtitle}` : ''}`, fontSize: 5, bold: true, color: getTxtColor(s), alignment: 'center' },
            ],
            fillColor: getFill(s),
        }));
        const LINE_H = 14;
        const mkLineCol = () => ({ canvas: [{ type: 'line', x1: 1, y1: 0, x2: 1, y2: LINE_H, lineWidth: 0.8, lineColor: '#9ca3af' }], width: 2 });
        const mkLeftMarker = (label, time) => ({
            columns: [mkLineCol(), { text: `${label}\n${time}`, fontSize: 4.5, color: '#6b7280', lineHeight: 1.3 }],
            columnGap: 1,
        });
        const mkRightMarker = (label, time) => ({
            columns: [{ text: `${label}\n${time}`, fontSize: 4.5, color: '#6b7280', lineHeight: 1.3, alignment: 'right' }, mkLineCol()],
            columnGap: 1,
        });
        ...
        return {
            ...
            layout: {
                hLineWidth: () => 0,
                vLineWidth: (i) => (i > 0 && i < segs.length ? 1 : 0),
                vLineColor: () => '#ffffff',
                paddingLeft: () => 0,
                paddingRight: () => 0,
                paddingTop: () => 2,
    ...
    filterReportByTeamPrefix(report, prefix) {
        const matches = (team) => team.toUpperCase().startsWith(prefix);
        return {
            ...report,
     ...
     */
    downloadPdf(section, safeName, helpers, expandedKeys, reportType = 'operacional') {
     ...
     */
    async generatePdfFile(section, safeName, helpers, expandedKeys, reportType = 'operacional') {
        const effectiveSection = { ...section, report: this.injectExpandedOrders(section.report, expandedKeys) };
        const docDef = reportType === 'analitico' ? this.buildAnalyticPdfDocDef(effectiveSection, helpers) : this.buildPdfDocDef(effectiveSection, helpers);
     ...
     */
    injectExpandedOrders(report, expandedKeys) {
        const mergeOrderExtrasAndSort = (analysisList, kpiName) => analysisList.map((a) => {
            const extras = (a.extraFlaggedOrders ?? []).filter((o) => (o.flags?.length ?? 0) > 0);
            if (extras.length === 0)
                return a;
        ...
        const mergeDayExtras = (analysisList, kpiName) => analysisList.map((a) => {
     ...
     */
    buildPdfDocDef(section, helpers) {
        const { report, title, subtitle, dateRangeLabel } = section;
        const barPct = (value, kpi) => {
        ...
        const metaLinePct = (kpi) => {
            const cfg = kpi.chartConfig;
            return cfg ? barPct(cfg.meta, kpi) : 50;
        };
        const isAbove = (kpi, value) => kpi.direction === 'higher-is-better' ? value >= kpi.metaTarget : value <= kpi.metaTarget;
        const fmt = (v, dec = 1) => v != null && Number.isFinite(v) ? v.toFixed(dec).replace('.', ',') : '–';
        ...
        const MUTED = '#94a3b8';
        const BG = '#f8f7f4';
        const th = (text) => ({ text, bold: true, fontSize: 7, color: GRAY, fillColor: BG, alignment: 'center', margin: [2, 3, 2, 3] });
        const td = (text, opts = {}) => ({ text, fontSize: 7.5, margin: [2, 3, 2, 3], ...opts });
        // Cover
        const cover = [
        ...
        const content = [...cover.filter(Boolean)];
        // Executive summary
        if (es) {
            content.push({ text: 'Resumo Executivo', style: 'sectionHeader', margin: [0, 0, 0, 10] }, {
                columns: [
                    { stack: [{ text: `${es.totalTeams}`, fontSize: 20, bold: true, color: DARK }, { text: 'Equipes', fontSize: 7, color: MUTED }], alignment: 'center' },
                    es.periodDays > 0 ? { stack: [{ text: `${es.periodDays}`, fontSize: 20, bold: true, color: DARK }, { text: 'Dias analisados', fontSize: 7, color: MUTED }], alignment: 'center' } : {},
                    { stack: [{ text: `${es.kpiAlerts.length}`, fontSize: 20, bold: true, color: RED }, { text: 'KPIs em alerta', fontSize: 7, color: MUTED }], alignment: 'center' },
                    { stack: [{ text: `${es.teamsBelowMetaCount}`, fontSize: 20, bold: true, color: RED }, { text: 'Equipes críticas', fontSize: 7, color: MUTED }], alignment: 'center' },
                ],
                columnGap: 12,
                margin: [0, 0, 0, 8],
            });
                ...
                es.kpiAlerts.forEach((a) => {
                    ...
                    content.push({
                        columns: [
                            ...
                            { text: `${a.teamsBelowMeta}/${es.totalTeams}`, bold: true, color: RED, fontSize: 7.5, width: 40, alignment: 'right' },
            ...
            const alertBadges = [];
            if (es.retornoBaseAlertCount > 0)
                alertBadges.push(`âš  ${es.retornoBaseAlertCount} equipe(s) com Retorno a Base acima da meta`);
            if (es.tmeImpAlertCount > 0)
                alertBadges.push(`âš  ${es.tmeImpAlertCount} equipe(s) com TME IMP acima da meta`);
        ...
        report.kpis.filter((kpi) => kpi.topTeams.length > 0 || kpi.opportunityTeams.length > 0).forEach((kpi) => {
            ...
            allTeams.forEach((t) => {
                ...
                kpiChartItems.push({
                    columns: [
                        ...
                        { text: `${fmt(t.value, dec)}${suffix}`, fontSize: 7.5, bold: true, color: valColor, width: 40, alignment: 'right' },
                    ],
                    columnGap: 8,
                    margin: [0, 0, 0, 3],
                });
                    ...
                    table: {
                        ...
                        body: [
                            ...
                            ...report.specialAnalysis.osDiaAnalysis.map((a) => [
                                td(a.team, { bold: true, alignment: 'left' }),
            ...
            // ---- Drill-down helpers ----
            const cardHeader = (team, badge, badgeRed = true) => ({
                columns: [
                    { text: team, bold: true, fontSize: 9, color: DARK, width: '*' },
                    { text: badge, bold: true, fontSize: 8, color: badgeRed ? RED : BLUE, width: 'auto', alignment: 'right' },
                ],
                margin: [0, 0, 0, 2],
            });
            const chipRow = (chips) => ({
            ...
            const tl = (...steps) => ({
                text: steps.flatMap((s, i) => i === 0
                    ? [{ text: s, color: GRAY }]
                    : [{ text: '  \u2014>  ', color: MUTED }, { text: s, color: GRAY }]),
                fontSize: 7,
                margin: [0, 1, 0, 1],
            });
            const orderHead = (nr_ordem, flags, labelFn, extra, isPrimeiraOs, customFlags = []) => {
            ...
            const minRuns = (text) => {
                ...
                return parts.flatMap((p, i) => p ? [i % 2 === 1 ? { text: p, bold: true, color: RED } : { text: p, color: DARK }] : []);
            };
            ...
            const alertWarnItemRuns = (label, bodyRuns) => {
                const warnUrl = helpers.renderSymbolDataUrl('\u26A0', 7, AMBER);
                const labelRuns = [{ text: label + ': ', bold: true, color: AMBER_DARK }, ...bodyRuns];
                if (warnUrl) {
                    return {
                        columns: [
                            { image: warnUrl, width: 7, height: 7, margin: [0, 0, 3, 0] },
            ...
            const cardDivider = () => ({ canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 0.8, lineColor: '#cbd5e1' }], margin: [0, 4, 0, 4] });
            const orderDivider = () => ({ canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 0.5, lineColor: '#e2e8f0' }], margin: [0, 3, 0, 3] });
            const indentBlock = (items, color, indent = 8) => ({
                table: {
                    widths: [2, '*'],
            ...
            // OS Dia drill-down
            if (kpi.kpi === 'OS Dia' && report.specialAnalysis.osDiaAnalysis?.length) {
                const osDiaList = report.specialAnalysis.osDiaAnalysis.filter((a) => (a.flaggedOrders?.length ?? 0) > 0 || !!a.idleAnalysis);
                const osDiaDrillHead = osDiaList.length > 0 ? drillHead('Análise Detalhada – OS Dia') : null;
                osDiaList.forEach((analysis, analysisIdx) => {
                    const chips = [
                        `OS/Dia ${fmt(analysis.osDiaValue)}`,
                        `Total OS: ${analysis.totalOrders} em ${analysis.totalJornadas} dias`,
                    ];
                    if (analysis.idleDays > 0)
                        chips.push(`Ocioso: ${analysis.idleDays} dias`);
                    if (analysis.summary?.countTrExceeds > 0)
                        chips.push(`Temp. Reparo>20% HD: ${analysis.summary.countTrExceeds}`);
                    if (analysis.summary?.countTlExceeds > 0)
                        chips.push(`Temp. Desloc.: ${analysis.summary.countTlExceeds}`);
                    if (analysis.summary?.countTempPrepAlto > 0)
                        chips.push(`Temp. Partida\u226510min: ${analysis.summary.countTempPrepAlto}`);
                    if (analysis.summary?.countSemOsAlto > 0)
                        chips.push(`SemOS\u226510min: ${analysis.summary.countSemOsAlto}`);
                    const teamItems = [chipRow(chips)];
                    ...
                    if (analysis.flaggedOrders?.length > 0) {
                        analysis.flaggedOrders.forEach((ev, evIdx, evArr) => {
                            const orderItems = [];
                            const ccParts = [];
                            if (ev.classe)
                                ccParts.push(`Classe: ${ev.classe}`);
                            if (ev.classe && ev.causa)
                                ccParts.push('  \u00b7  ');
                            if (ev.causa)
                                ccParts.push(`Causa: ${ev.causa}`);
                            ...
                            if (ccParts.length > 0)
                                orderItems.push({ text: ccParts.join(''), fontSize: 7, color: GRAY, margin: [0, 0, 0, 2] });
                            const osDiaTl = this.buildTimelinePdfBlock(ev);
                            if (osDiaTl)
                                orderItems.push(osDiaTl);
                            if (ev.flags?.includes('tr_excede_hd'))
                                orderItems.push(alertItem(`Tempo de Reparo alto: ${helpers.osDiaAlertBody('tr_excede_hd', ev)}`));
                            if (ev.flags?.includes('tl_excede_hd'))
                                orderItems.push(alertItem(`Tempo de Deslocamento alto: ${helpers.osDiaAlertBody('tl_excede_hd', ev)}`));
                            if (ev.flags?.includes('temp_prep_alto'))
                                orderItems.push(alertItem(`Tempo de Partida/OS: ${helpers.osDiaAlertBody('temp_prep_alto', ev)}`));
                            if (ev.flags?.includes('triagem_alto'))
                                orderItems.push(alertItem(`2º Desp. | Prioritário: ${helpers.osDiaAlertBody('triagem_alto', ev)}`));
                            if (ev.flags?.includes('primeiro_desloc_alto'))
                                orderItems.push(alertItem(`1º Desloc.: ${helpers.osDiaAlertBody('primeiro_desloc_alto', ev)}`));
                            if (ev.flags?.includes('sem_os_alto') && ev.sem_os_details?.length) {
                                ...
                                ev.sem_os_details.filter((d) => d.type !== 'fim_jornada').forEach((d, di) => {
                            ...
                            if (ev.flags?.includes('retorno_excedente')) {
                                const fjDetail = ev.sem_os_details?.find((d) => d.type === 'fim_jornada');
                                if (fjDetail) {
                            ...
                            const customFlags = [];
                            if (ev.sem_os_details?.find((d) => d.type === 'entre_os' && d.interval_discounted && d.min >= 10)) {
                            ...
                            const orderBlock = [orderHead(ev.nr_ordem, ev.flags ?? [], (f) => helpers.osDiaFlagLabel(f), ev.date_ref || undefined, !ev.prev_liberada, customFlags)];
                            if (orderItems.length > 0)
                                orderBlock.push(indentBlock(orderItems, '#94a3b8', 6));
                            teamItems.push({ stack: orderBlock, unbreakable: true });
                            if (evIdx < evArr.length - 1)
                                teamItems.push(orderDivider());
                        });
                    }
            ...
            if (kpi.kpi === 'Eficiência' && kpi.evidenceAnalysis?.length) {
                const efList = kpi.evidenceAnalysis.filter((a) => (a.flaggedOrders?.length ?? 0) > 0 || (a.summary?.countTempoPadraoVazio ?? 0) > 0);
                const efDrillHead = efList.length > 0 ? drillHead('Análise Detalhada – Eficiência (Top 3 e 3 Abaixo do Padrão)') : null;
                efList.forEach((analysis, analysisIdx) => {
                    ...
                    if (analysis.avgTempoPadraoMin > 0)
                        chips.push(`T. Padrão Médio: ${analysis.avgTempoPadraoMin?.toFixed(0)} min`);
                    if (analysis.summary?.countDeslocamentoCurto > 0)
                        chips.push(`TL Curto: ${analysis.summary.countDeslocamentoCurto}`);
                    const teamItems = [chipRow(chips)];
                    ...
                    analysis.flaggedOrders?.forEach((ev, evIdx, evArr) => {
                        ...
                        const efCcParts = [];
                        if (ev.classe)
                            efCcParts.push(`Classe: ${ev.classe}`);
                        if (ev.classe && ev.causa)
                            efCcParts.push('  \u00b7  ');
                        if (ev.causa)
                            efCcParts.push(`Causa: ${ev.causa}`);
                        ...
                        if (_efDesp && ev.prev_liberada) {
                            ...
                            if (_efPrevTs > 0 && _efDespTs > 0 && _efPrevTs > _efDespTs) {
                                if (efCcParts.length > 0)
                                    efCcParts.push('  \u00b7  ');
                                efCcParts.push(`Desp.: ${extractTime(_efDesp)}`);
                            }
                        }
                        if (efCcParts.length > 0)
                            orderItems.push({ text: efCcParts.join(''), fontSize: 7, color: GRAY, margin: [0, 0, 0, 2] });
                        const efTl = this.buildTimelinePdfBlock(ev, true, true);
                        if (efTl)
                            orderItems.push(efTl);
                        if (ev.flags?.includes('tr_muito_baixo'))
                            orderItems.push(alertItem(`Tempo de Reparo muito baixo: ${helpers.eficienciaAlertBody('tr_muito_baixo', ev)}`));
                        if (ev.flags?.includes('deslocamento_curto'))
                            orderItems.push(alertItem(`Deslocamento (TL) muito curto: ${helpers.eficienciaAlertBody('deslocamento_curto', ev)}`));
                        if (ev.flags?.includes('tr_excede_hd'))
                            orderItems.push(alertItem(`Tempo de Reparo alto: ${helpers.eficienciaAlertBody('tr_excede_hd', ev)}`));
                        if (ev.flags?.includes('tempo_padrao_vazio'))
                            orderItems.push(alertItem(`Tempo Padrão ausente: ${helpers.eficienciaAlertBody('tempo_padrao_vazio', ev)}`));
                        const orderBlock = [orderHead(ev.nr_ordem, ev.flags ?? [], (f) => helpers.eficienciaFlagLabel(f), ev.date_ref || undefined, !ev.prev_liberada)];
                        if (orderItems.length > 0)
                            orderBlock.push(indentBlock(orderItems, '#94a3b8', 6));
                        teamItems.push({ stack: orderBlock, unbreakable: true });
                        if (evIdx < evArr.length - 1)
            ...
            if (kpi.kpi === 'Utilização' && report.specialAnalysis.utilizacaoAnalysis?.length) {
                const utilList = report.specialAnalysis.utilizacaoAnalysis.filter((a) => (a.flaggedOrders?.length ?? 0) > 0 || !!a.idleAnalysis);
                const utilDrillHead = utilList.length > 0 ? drillHead('Análise Detalhada – Utilização (3 Abaixo do Padrão)') : null;
                utilList.forEach((analysis, analysisIdx) => {
                    const chips = [
                        `Utilização: ${analysis.utilizacaoValue}%`,
                        `Meta: ${analysis.metaTarget}%`,
                        `Total OS: ${analysis.totalOrders} em ${analysis.totalJornadas} dias`,
                    ];
                    if (analysis.jornadasAbaixoMeta > 0)
                        chips.push(`Jornadas < meta: ${analysis.jornadasAbaixoMeta}/${analysis.totalJornadas}`);
                    if (analysis.idleDays > 0)
                        chips.push(`Ocioso: ${analysis.idleDays} dias`);
                    if (analysis.summary?.countTempPrepAlto > 0)
                        chips.push(`Temp. Partida\u226510min: ${analysis.summary.countTempPrepAlto}`);
                    ...
                    analysis.flaggedOrders?.forEach((ev, evIdx, evArr) => {
                        ...
                        const utilCcParts = [];
                        if (ev.classe)
                            utilCcParts.push(`Classe: ${ev.classe}`);
                        if (ev.classe && ev.causa)
                            utilCcParts.push('  \u00b7  ');
                        if (ev.causa)
                            utilCcParts.push(`Causa: ${ev.causa}`);
                        ...
                        if (_utilDesp && ev.prev_liberada) {
                            ...
                            if (_utilPrevTs > 0 && _utilDespTs > 0 && _utilPrevTs > _utilDespTs) {
                                if (utilCcParts.length > 0)
                                    utilCcParts.push('  \u00b7  ');
                                utilCcParts.push(`Desp.: ${extractTime(_utilDesp)}`);
                            }
                        }
                        if (utilCcParts.length > 0)
                            orderItems.push({ text: utilCcParts.join(''), fontSize: 7, color: GRAY, margin: [0, 0, 0, 2] });
                        const utilTl = this.buildTimelinePdfBlock(ev);
                        if (utilTl)
                            orderItems.push(utilTl);
                        if (ev.flags?.includes('tr_excede_hd'))
                            orderItems.push(alertItem(`Tempo de Reparo alto: ${helpers.osDiaAlertBody('tr_excede_hd', ev)}`));
                        if (ev.flags?.includes('temp_prep_alto'))
                            orderItems.push(alertItem(`Tempo de Partida/OS: ${helpers.osDiaAlertBody('temp_prep_alto', ev)}`));
            ...
            if (kpi.kpi === 'TME IMP' && kpi.tmeImpAnalysis?.length) {
                const tmeList = kpi.tmeImpAnalysis.filter((a) => (a.flaggedOrders?.length ?? 0) > 0);
                const tmeDrillHead = tmeList.length > 0 ? drillHead('Análise Detalhada – TME IMP (Ordens com TME Elevado)') : null;
                tmeList.forEach((analysis, analysisIdx) => {
                    const chips = [
                    ...
                    if (analysis.summary?.countTmeMuitoAlto > 0)
                        chips.push(`TME\u22651.5\u00d7avg: ${analysis.summary.countTmeMuitoAlto}`);
                    if (analysis.summary?.countSemDeslocamento > 0)
                        chips.push(`Sem desloc.: ${analysis.summary.countSemDeslocamento}`);
                    const teamItems = [chipRow(chips)];
                    analysis.flaggedOrders?.forEach((ev, evIdx, evArr) => {
                        const orderItems = [];
                        if (ev.classe || ev.causa) {
                        ...
                        if (ev.flags?.includes('tme_muito_alto'))
                            orderItems.push(alertItem(`TME IMP elevado: ${helpers.tmeImpAlertBody('tme_muito_alto', ev)}`));
                        if (ev.flags?.includes('sem_deslocamento'))
                            orderItems.push(alertItem(`Sem registro de deslocamento: ${helpers.tmeImpAlertBody('sem_deslocamento', ev)}`));
                        if (ev.flags?.includes('sem_execucao'))
                            orderItems.push(alertItem(`Sem TR Ordem: ${helpers.tmeImpAlertBody('sem_execucao', ev)}`));
                        const orderBlock = [orderHead(ev.nr_ordem, ev.flags ?? [], (f) => helpers.tmeImpFlagLabel(f), ev.date_ref || undefined)];
                        if (orderItems.length > 0)
                            orderBlock.push(indentBlock(orderItems, '#94a3b8', 6));
                        teamItems.push({ stack: orderBlock, unbreakable: true });
                        if (evIdx < evArr.length - 1)
            ...
            if (kpi.kpi === '1º Login' && kpi.primeiroLoginAnalysis?.length) {
                const loginList = kpi.primeiroLoginAnalysis.filter((a) => (a.flaggedDays?.length ?? 0) > 0);
                const loginDrillHead = loginList.length > 0 ? drillHead('Análise Detalhada – 1º Login (Dias Acima da Meta)') : null;
                loginList.forEach((analysis, analysisIdx) => {
                    const chips = [
                    ...
                    if (analysis.summary?.countLoginMuitoTardio > 0)
                        chips.push(`Login>16min: ${analysis.summary.countLoginMuitoTardio}`);
                    const teamItems = [chipRow(chips)];
                    analysis.flaggedDays?.forEach((ev, evIdx, evArr) => {
                        const dayItems = [];
                        dayItems.push(tl('Inicio Cal.', ev.inicio_calendario || '\u2014', `Log In: ${ev.log_in_corrigido || '\u2014'}`));
                        if (ev.flags?.includes('login_muito_tardio'))
                            dayItems.push(alertItem(`Login muito tardio: ${helpers.loginAlertBody('login_muito_tardio', ev)}`));
                        else if (ev.flags?.includes('login_tardio'))
                            dayItems.push(alertItem(`Login tardio: ${helpers.loginAlertBody('login_tardio', ev)}`));
                        teamItems.push({ stack: [
                                {
                                    text: [
                                        { text: ev.date_ref || '\u2014', bold: true, fontSize: 7.5, color: DARK },
                                        { text: '    ' },
                                        ...((ev.flags ?? []).flatMap((f, i) => [
            ...
            if (kpi.kpi === '1º Desloc.' && kpi.primeiroDeslocAnalysis?.length) {
                const deslocList = kpi.primeiroDeslocAnalysis.filter((a) => (a.flaggedDays?.length ?? 0) > 0);
                const deslocDrillHead = deslocList.length > 0 ? drillHead('Análise Detalhada – 1º Desloc. (Dias Acima da Meta)') : null;
                deslocList.forEach((analysis, analysisIdx) => {
                    const chips = [
                    ...
                    if (analysis.summary?.countDeslocMuitoLento > 0)
                        chips.push(`Desloc.>37min: ${analysis.summary.countDeslocMuitoLento}`);
                    if (analysis.summary?.countSemDeslocRegistrado > 0)
                        chips.push(`Sem registro: ${analysis.summary.countSemDeslocRegistrado}`);
                    if (analysis.summary?.countDespachioTardio > 0)
                        chips.push(`Despacho tardio: ${analysis.summary.countDespachioTardio}`);
                    const teamItems = [chipRow(chips)];
                    analysis.flaggedDays?.forEach((ev, evIdx, evArr) => {
                        const dayItems = [];
                        const deslocTl = this.buildTimelinePdfBlock(ev);
                        if (deslocTl)
                            dayItems.push(deslocTl);
                        if (ev.flags?.includes('despacho_tardio'))
                            dayItems.push(alertItem(`Despacho tardio: ${helpers.deslocAlertBody('despacho_tardio', ev)}`));
                        if (ev.flags?.includes('desloc_muito_lento'))
                            dayItems.push(alertItem(`1º Desloc.: ${helpers.deslocAlertBody('desloc_muito_lento', ev)}`));
                        else if (ev.flags?.includes('desloc_lento'))
                            dayItems.push(alertItem(`1º Desloc.: ${helpers.deslocAlertBody('desloc_lento', ev)}`));
                        if (ev.flags?.includes('triagem_alto'))
                            dayItems.push(alertItem(`2º Desp. | Prioritário: ${helpers.deslocAlertBody('triagem_alto', ev)}`));
                        if (ev.flags?.includes('sem_desloc_registrado'))
                            dayItems.push(alertItem(`Sem deslocamento registrado: ${helpers.deslocAlertBody('sem_desloc_registrado', ev)}`));
                        if (ev.nr_ordem_despacho_anterior) {
                            const horaFmt = (ev.hora_despacho_anterior || '').replace(/^(\d{2}\/\d{2})\/\d{4}\s+(\d{2}:\d{2}).*$/, '$1 $2');
            ...
            if (kpi.kpi === 'Retorno Base' && kpi.retornoBaseAnalysis?.length) {
                const retornoList = kpi.retornoBaseAnalysis.filter((a) => (a.flaggedDays?.length ?? 0) > 0);
                const retornoDrillHead = retornoList.length > 0 ? drillHead('Análise Detalhada – Retorno Base (Dias Acima da Meta)') : null;
                retornoList.forEach((analysis, analysisIdx) => {
                    const chips = [
                    ...
                    if (analysis.summary?.countRetornoMuitoAlto > 0)
                        chips.push(`Retorno>60min: ${analysis.summary.countRetornoMuitoAlto}`);
                    const teamItems = [chipRow(chips)];
                    analysis.flaggedDays?.forEach((ev, evIdx, evArr) => {
                        const dayItems = [];
                        dayItems.push(tl('Ultima OS Lib.', ev.hora_ultima_ordem || '\u2014', `Log Off: ${ev.log_off_corrigido || '\u2014'}`));
                        if (ev.flags?.includes('retorno_divergente'))
                            dayItems.push(alertItem(`Divergência detectada: ${helpers.retornoAlertBody('retorno_divergente', ev)}`));
                        if (ev.flags?.includes('retorno_muito_alto'))
                            dayItems.push(alertItem(`Retorno muito alto: ${helpers.retornoAlertBody('retorno_muito_alto', ev)}`));
                        else if (ev.flags?.includes('retorno_alto'))
                            dayItems.push(alertItem(`Retorno acima da meta: ${helpers.retornoAlertBody('retorno_alto', ev)}`));
                        teamItems.push({ stack: [
                                {
                                    text: [
                                        { text: ev.date_ref || '\u2014', bold: true, fontSize: 7.5, color: DARK },
        ...
        if (report.teamScorecard.length > 0) {
            content.push({ canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 0.8, lineColor: '#cbd5e1' }], margin: [0, 14, 0, 0] }, { text: 'Scorecard por Equipe', style: 'sectionHeader', margin: [0, 16, 0, 6] }, { text: 'Todos os KPIs por equipe. Azul = meta atingida, vermelho = abaixo da meta.', fontSize: 7.5, color: GRAY, margin: [0, 0, 0, 8] }, {
                table: {
                    headerRows: 1,
                    ...
                    body: [
                        ...
                        ...report.teamScorecard.map((row) => [
                            td(row.team, { bold: true, alignment: 'left', fillColor: row.kpisBelowMeta >= 4 ? '#fff1f2' : row.kpisBelowMeta === 3 ? '#fffbeb' : null }),
        ...
        if (report.deviations.mostRecurring.length > 0) {
            content.push({ canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 0.8, lineColor: '#cbd5e1' }], margin: [0, 14, 0, 0], pageBreak: 'after' }, { text: 'Desvios de Padrão Operacional', style: 'sectionHeader', margin: [0, 16, 0, 8] }, {
                columns: [
                    {
                        stack: [
                            ...
                            { text: 'Por Equipe', bold: true, fontSize: 8, color: GRAY, margin: [0, 0, 0, 6] },
                            {
                                table: {
                                    ...
                                    body: report.deviations.teamBreakdown.map((t) => [
                                        td(t.team, { bold: true, alignment: 'left' }),
        ...
        if (report.specialAnalysis.actionPlan.length > 0) {
            content.push({ canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 0.8, lineColor: '#cbd5e1' }], margin: [0, 14, 0, 0] }, { text: 'Plano de Acao por Equipe', style: 'sectionHeader', margin: [0, 16, 0, 8] });
            ...
            for (let i = 0; i < plans.length; i += 2) {
                const makeCard = (plan) => ({
                    stack: [
                        ...
                        ...plan.issues.map((iss) => ({ text: `! ${iss}`, fontSize: 7.5, color: RED, margin: [0, 0, 0, 2] })),
                        ...plan.recommendations.map((r) => ({ text: `> ${r}`, fontSize: 7, color: DARK, margin: [4, 0, 0, 2] })),
                    ],
                    margin: [0, 0, 0, 8],
                });
                content.push({
     ...
     */
    buildAnalyticPdfDocDef(section, helpers) {
        const { report, title, subtitle, dateRangeLabel } = section;
        const fmt = (v, dec = 1) => v != null && Number.isFinite(v) ? v.toFixed(dec).replace('.', ',') : '–';
        const today = new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' });
        const es = report.executiveSummary;
        ...
        report.kpis.forEach((kpi) => {
            ...
            if (cd && cd.trendLines && cd.trendLines.length > 0) {
                const w = 500;
                const h = 40;
                const minVal = Math.min(...cd.trendLines.flatMap((tl) => tl.points.map((p) => p.value)), kpi.metaTarget) * 0.9;
                const maxVal = Math.max(...cd.trendLines.flatMap((tl) => tl.points.map((p) => p.value)), kpi.metaTarget) * 1.1;
                const toX = (di) => (cd.days.length > 1 ? (di / (cd.days.length - 1)) * w : w / 2);
                const toY = (v) => h - ((v - minVal) / (maxVal - minVal)) * h;
                ...
                // Draw each line
                const legendItems = [];
                cd.trendLines.forEach((tl) => {
                    const points = tl.points.map((pt) => {
                        const di = cd.days.findIndex((d) => d.label === pt.label);
                ...
                const w = 500;
                const h = 40;
                const toX = (i) => (trendPts.length > 1 ? (i / (trendPts.length - 1)) * w : w / 2);
                const toY = (v) => h - ((v - minVal) / (maxVal - minVal)) * h;
            ...
            const renderTeamList = (teams, isTop) => {
                if (teams.length === 0)
                    return [{ text: 'Nenhuma equipe nesta categoria.', fontSize: 7, color: MUTED, italics: true }];
                return teams.map(t => ({
                    columns: [
                        { text: t.team, fontSize: 7, color: DARK, width: '*' },
                        { text: `${fmt(t.value, dec)}${suffix}`, fontSize: 7, bold: true, color: isTop ? GREEN : RED, alignment: 'right', width: 30 }
                    ],
                    margin: [0, 2, 0, 2]
                }));
            };
        ...
        if (report.teamScorecard && report.teamScorecard.length > 0) {
            ...
            const th = (text) => ({ text, bold: true, fontSize: 7, color: GRAY, fillColor: BG, alignment: 'center', margin: [2, 3, 2, 3] });
            const td = (text, opts = {}) => ({ text, fontSize: 7.5, margin: [2, 3, 2, 3], alignment: 'center', ...opts });
            const scoreRows = report.teamScorecard.map((t, index) => [
                td(`${index + 1}º`),
                td(t.team, { bold: true, alignment: 'left' }),
                td(`${t.diasTrabalhados ?? '-'}`, { alignment: 'center' }),
                td(fmt(t.score), { bold: true, color: BLUE, alignment: 'center' }),
                td(`${t.kpisBelowMeta}`, { color: t.kpisBelowMeta > 0 ? RED : GREEN, alignment: 'center' }),
...
DashboardPdfService = DashboardPdfService_1 = __decorate([
