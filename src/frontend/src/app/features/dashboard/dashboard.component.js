...
import { Component, NgZone, ViewChild, computed, inject, signal } from '@angular/core';
...
let DashboardComponent = class DashboardComponent {
    constructor() {
        ...
        this.pdfService = inject(DashboardPdfService);
        this.chartService = inject(DashboardChartService);
        this.sanitizer = inject(DomSanitizer);
        this.allOption = ALL_OPTION;
        this.loading = signal(false);
        this.progressMessage = signal('');
        this.progressLog = signal([]);
        this.latestFilterLog = computed(() => {
            const log = this.progressLog();
            return log.length > 0 ? log[log.length - 1] : '';
        });
        this.generatingReport = computed(() => this.progressMessage().toLowerCase().startsWith('gerando relat'));
        this.errorMessage = signal('');
        this.filterDrawerOpen = signal(false);
        this.selectedTrendLine = signal(null);
        this.exportModalOpen = signal(false);
        this.exportModalStep = signal('mode');
        this.exportModeType = signal('proprias');
        this.exportLoading = signal(false);
        this.exportError = signal('');
        this.includeAttentionMessage = signal(true);
        /** Arquivos prontos aguardando gesto do usuário para navigator.share. */
        this.pendingShareFiles = signal(null);
        this.pendingShareText = signal(null);
        this.selectedDayPerKpi = signal({});
        this.expandedTeamDetails = signal(null);
        this.kpiStates = signal({});
        this.pendingShareMode = signal(null);
        /** Modo cujo botão Compartilhar está carregando no momento. */
        this.shareModeLoading = signal(null);
        this.reportBaseOptions = [];
        this.availablePropriasBases = [];
        this.availableParceirasBases = [];
        this.reportBasePrefixMap = {};
        this.basesConfig = null;
        this.availableDates = [];
        this.reportBarHidden = signal(true);
        this.reportData = signal(null);
        this.reportTitle = signal(DEFAULT_REPORT_TITLE);
        this.reportType = signal('operacional');
        this.selectFilters = signal([]);
        this.analiticoFilters = signal([]);
        this.operacionalFilters = signal([]);
        this.reportFilterStates = computed(() => this.reportType() === 'analitico' ? this.analiticoFilters() : this.operacionalFilters());
        this.pendingScrollToKpi = signal(null);
        this.openDropdownKey = signal(null);
        this.dropdownSearch = signal('');
        this.dayRange = signal({ min: 1, max: 31 });
        this.resolvedDayRange = computed(() => {
        ...
        this.reportTypeLabel = computed(() => {
            const opt = this.reportTypeOptions.find((o) => o.value === this.reportType());
            return opt ? opt.label : 'Operacional';
        });
        this.filtersVisible = computed(() => this.selectFilters().length > 0);
        this.periodFilters = computed(() => this.selectFilters().filter((filter) => filter.key === 'ano' || filter.key === 'mes'));
        this.secondaryFilters = computed(() => this.selectFilters().filter((filter) => filter.key !== 'ano' && filter.key !== 'mes'));
        this.dayLimit = computed(() => {
        ...
        this.dayLimitMin = computed(() => {
            const year = this.periodFilters().find((filter) => filter.key === 'ano')?.value ?? [];
            const month = this.periodFilters().find((filter) => filter.key === 'mes')?.value ?? [];
            return this.dayLimitForMonthPosition(year, month, 'first');
        });
        this.periodRangeLabel = computed(() => {
        ...
        this.multiMonthSelected = computed(() => {
            const monthFilter = this.periodFilters().find((f) => f.key === 'mes');
            return (monthFilter?.value ?? []).filter((m) => m !== ALL_OPTION).length > 1;
        });
        this.sortedActiveMonthIndexes = computed(() => {
            const monthFilter = this.periodFilters().find((f) => f.key === 'mes');
        ...
        this.sliderMin = computed(() => this.dayRange().min);
        // Raw absolute position of the right thumb (thumb 2)
        this.sliderMax = computed(() => {
            if (!this.multiMonthSelected())
                return this.dayRange().max;
        ...
        this.dayMinLabel = computed(() => {
            const indexes = this.sortedActiveMonthIndexes();
            if (indexes.length <= 1)
                return String(this.resolvedDayRange().min);
        ...
        this.dayMaxLabel = computed(() => {
            const indexes = this.sortedActiveMonthIndexes();
            if (indexes.length <= 1)
                return String(this.resolvedDayRange().max);
        ...
        this.boundWindowMouseMove = (event) => {
        ...
        this.analyticSelectedTeam = signal({});
        this.analyticSelectedDay = signal({});
        this.analyticLegendOpen = signal({});
        this.analyticSearch = signal({});
        // ─── "Ver mais..." per team state ────────────────────────────────
        /** Key format: `{kpiName}|{team}` — true when expanded. */
        this.expandedEvidenceTeams = signal({});
        // ─── "Ver mais..." per date-ref group state (Legacy for Day-based KPIs) ───
        /** Key format: `{kpiName}|{team}|{dateRef}` — true when expanded. */
        this.expandedEvidenceDates = signal({});
    }
    toggleTrendLine(base, teamType) {
    ...
    setReportFilterStates(newState, mode) {
    ...
    ngOnInit() {
        this.api.getBasesConfig().subscribe(config => {
            ...
            const updateFilters = (filters, mode) => {
        ...
        const overrides = saved ? new Map(Object.entries(saved.filters)) : undefined;
    ...
    barWidthPct(value, kpiName, kpi) {
        const cfg = kpi?.chartConfig;
        if (!cfg || !Number.isFinite(value))
            return 0;
    ...
    kpiMetaPct(kpiName, metaTarget, kpi) {
     ...
     */
    async shareMode(mode) {
        if (this.shareModeLoading())
            return;
    ...
    async fetchAndBuildFiles(mode) {
        if (mode === 'current') {
            const filters = this.buildReportFiltersPayload();
            const result = await firstValueFrom(this.api.exportData({
            ...
            const teamType = mode === 'proprias' ? 'propria' : 'parceira';
            ...
            const results = await firstValueFrom(forkJoin(basesToExport.map(base => this.api.exportData({ reportFilters: { bases: [base], teamTypes: [teamType], dates: currentFilters.dates } }))));
            ...
            for (const team of topTeams) {
                ...
                for (const order of flaggedOrders) {
                    if (order.flags?.includes('temp_prep_alto') && order.temp_prep_os_min) {
                        ...
                        countPrep++;
                        if (order.temp_prep_os_min > maxPrep)
                            maxPrep = order.temp_prep_os_min;
                    }
                ...
                const motivos = [];
                if (util < 50)
                    motivos.push('Utilização < 50%');
                if ((osDiaMap.get(team.team)?.idleAnalysis?.idlePct ?? 0) >= 15)
                    motivos.push('Ociosidade ≥ 15%');
                if (team.kpisBelowMeta >= 4)
                    motivos.push('4+ KPIs abaixo da meta');
                let desviosText = '';
                const infos = [];
                ...
                if (infos.length > 0)
                    desviosText += infos.join(' | ');
                if (!teveIntervalo4a6) {
                    desviosText += (desviosText ? ' | ' : '') + '*Intervalo NÃO declarado* (ou fora de 4h-6h)';
                }
                let linha = `• ${team.team} (Motivo: ${motivos.join(', ')}):\n`;
                if (kpisImpactados.length > 0)
                    linha += `    ↳ KPIs Impactados: ${kpisImpactados.join(', ')}\n`;
                if (desviosText)
                    linha += `    ↳ Desvios: ${desviosText}\n`;
                text += linha;
            }
            text += '\n';
        }
     ...
     */
    exportWithMode(mode, shareAfter = false) {
        ...
        this.exportError.set('');
        const currentFilters = this.buildReportFiltersPayload();
        const requests = basesToExport.map((base) => this.api.exportData({ reportFilters: { bases: [base], teamTypes: [teamType], dates: currentFilters.dates } }));
    ...
    exportBase(base) {
        ...
        const exportType = mode === 'proprias' ? 'proprias' : 'parceiras';
        const teamType = mode === 'proprias' ? 'propria' : 'parceira';
        const typeLabel = mode === 'proprias' ? 'Equipes Próprias' : 'Equipes Parceiras';
        this.exportLoading.set(true);
        this.exportError.set('');
     ...
     */
    buildPdfFileName(section, exportType) {
        const monthAbbrevToNum = {
            jan: '01', fev: '02', mar: '03', abr: '04', mai: '05', jun: '06',
            jul: '07', ago: '08', set: '09', out: '10', nov: '11', dez: '12',
        };
     ...
     */
    downloadPdfDirect(section, exportType) {
     ...
     */
    buildPdfFileForShare(section, exportType) {
        const { safeName, dateRangeLabel } = this.buildPdfFileName(section, exportType);
        const expandedKeys = exportType === 'atual' ? this.getExpandedEvidenceKeysSet() : undefined;
    ...
    buildPdfHelpers() {
        return {
            renderEmojiDataUrl: (e, s) => this.pdfService.renderEmojiDataUrl(e, s),
            renderSymbolDataUrl: (sym, s, c) => this.pdfService.renderSymbolDataUrl(sym, s, c),
            stripEmojiForPdf: (t) => this.pdfService.stripEmojiForPdf(t),
            semOsDetailLabel: (d, nrOrdemDespachoAnterior) => this.semOsDetailLabel(d, nrOrdemDespachoAnterior),
            semOsDetailBody: (d, nrOrdemDespachoAnterior) => this.semOsDetailBody(d, nrOrdemDespachoAnterior),
            osDiaFlagLabel: (f) => this.osDiaFlagLabel(f),
            eficienciaFlagLabel: (f) => this.eficienciaFlagLabel(f),
            tmeImpFlagLabel: (f) => this.tmeImpFlagLabel(f),
            loginFlagLabel: (f) => this.loginFlagLabel(f),
            deslocFlagLabel: (f) => this.deslocFlagLabel(f),
            retornoFlagLabel: (f) => this.retornoFlagLabel(f),
            analyticChartData: (kpi) => this.analyticChartData(kpi),
            osDiaAlertBody: (flag, ev) => this.osDiaAlertBody(flag, ev),
            eficienciaAlertBody: (flag, ev) => this.eficienciaAlertBody(flag, ev),
            tmeImpAlertBody: (flag, ev) => this.tmeImpAlertBody(flag, ev),
            loginAlertBody: (flag, ev) => this.loginAlertBody(flag, ev),
            deslocAlertBody: (flag, ev) => this.deslocAlertBody(flag, ev),
            retornoAlertBody: (flag, ev) => this.retornoAlertBody(flag, ev),
        };
    }
    openFilterDrawer() {
     ...
     */
    allDateGroupsForKpi(top, extra) {
        const topGroups = this.groupByDateRef(top);
        const topDates = new Set(topGroups.map(g => g.dateRef));
        const extraOnlyDates = new Set((extra ?? []).map(o => (o.date_ref ?? '').trim() || '—').filter(d => !topDates.has(d)));
        const parseDate = (s) => {
            const p = s.split('/');
            return p.length >= 3 ? +p[2] * 10000 + +p[1] * 100 + +p[0] : 0;
        };
    ...
    getExtraForDate(extra, dateRef) {
        return (extra ?? []).filter((o) => ((o.date_ref ?? '').trim() || '—') === dateRef);
    }
    hasExtraForDate(extra, dateRef) {
    ...
    clearAnalyticTeam(kpiIndex) {
        this.analyticSelectedTeam.update((cur) => ({ ...cur, [kpiIndex]: null }));
        this.analyticSelectedDay.update((cur) => ({ ...cur, [kpiIndex]: null }));
    }
    selectAnalyticPoint(team, dayLabel, kpiIndex, ev) {
        ev.stopPropagation();
        const wasSelected = (this.analyticSelectedTeam()[kpiIndex] ?? null) === team;
    ...
    parseTeamBase(teamName) {
        ...
        for (const polo of config.polos) {
            if (polo.matchType === 'direct_prefix') {
                for (const base of polo.bases) {
                    if (base.propria?.some((p) => upper.startsWith(p.toUpperCase())))
                        return { base: base.name, teamType: 'propria' };
                    if (base.parceira?.some((p) => upper.startsWith(p.toUpperCase())))
                        return { base: base.name, teamType: 'parceira' };
                }
            }
            else if (polo.matchType === 'infix_type_with_base_prefix') {
                for (const base of polo.bases) {
                    if (base.prefixes?.some((p) => upper.startsWith(p.toUpperCase()))) {
                        let tType = null;
                        if (polo.typeIdentifiers?.propria.some((inf) => upper.includes(inf.toUpperCase())))
                            tType = 'propria';
                        else if (polo.typeIdentifiers?.parceira.some((inf) => upper.includes(inf.toUpperCase())))
                            tType = 'parceira';
    ...
    toggleKpiDay(kpiId, day, base, teamType) {
    ...
    redirectFromExpandedToOperacional(dateLabel) {
        ...
        const updatedOpFilters = currentOpFilters.map(filter => {
            if (filter.key === 'reportDataRef')
                return { ...filter, value: [fullDate] };
            if (filter.key === 'reportEquipe')
                return { ...filter, value: [data.team] };
            return filter;
        });
    ...
    getTeamDailyDetails(data) {
        if (!data || !data.perTeamDailyData)
            return [];
        const teamData = data.perTeamDailyData.find((t) => t.team === data.team);
        if (!teamData || !teamData.dailyPoints)
            return [];
        return teamData.dailyPoints.filter((p) => p.value !== undefined).map((p) => ({ date: p.date, value: p.value }));
    }
    isTeamInBottomOsDia(teamName) {
        const report = this.reportData();
        if (!report || !report.kpis)
            return false;
        const osDiaKpi = report.kpis.find((k) => k.kpi === 'OS Dia');
        if (!osDiaKpi || !osDiaKpi.opportunityTeams)
            return false;
        return osDiaKpi.opportunityTeams.some((t) => t.team === teamName);
    }
    getFilteredDestaques(kpi, cd) {
        const selectedDay = this.selectedDayPerKpi()[kpi.kpi] || null;
        let rawTop = [];
        let rawBottom = [];
        let selectedDayAvg = null;
        if (!cd || !cd.trendLines || cd.trendLines.length === 0) {
            ...
            const filterTeamByCombo = (teamName) => {
                if (!selected)
                    return true;
                const info = this.parseTeamBase(teamName);
                if (!info)
                    return false;
                if (activeCombos.has(`${info.base}|All`))
                    return true;
                if (activeCombos.has(`Média Global|${info.teamType}`))
                    return true;
                if (activeCombos.has(`Média Global|All`))
                    return true;
            ...
            if (selectedDay && kpi.perTeamDailyData) {
                for (const t of kpi.perTeamDailyData) {
                    if (!filterTeamByCombo(t.team))
                        continue;
                    const pt = t.dailyPoints.find((p) => p.date === selectedDay || p.date.startsWith(selectedDay + '/') || selectedDay.startsWith(p.date + '/'));
                ...
                filteredScores = (kpi.scores || []).filter((s) => filterTeamByCombo(s.team));
            }
            const direction = kpi.direction || 'higher-is-better';
            filteredScores.sort((a, b) => direction === 'higher-is-better' ? b.rawValue - a.rawValue : a.rawValue - b.rawValue);
            const opportunityPool = filteredScores.filter((s) => direction === 'higher-is-better' ? s.rawValue < kpi.metaTarget : s.rawValue > kpi.metaTarget);
            if (selectedDay) {
                const trendLine = cd.trendLines.find((tl) => selected ? `${tl.base}|${tl.teamType}` === selected : tl.base === 'Global');
                if (trendLine) {
                    const pt = trendLine.points.find((p) => p.label === selectedDay);
                    if (pt)
                        selectedDayAvg = pt.value;
                }
            }
            rawTop = filteredScores.slice(0, 3).map((s) => ({ team: s.team, value: s.rawValue }));
            rawBottom = opportunityPool.slice(-3).reverse().map((s) => ({ team: s.team, value: s.rawValue }));
        }
        ...
        if (report && report.specialAnalysis) {
            if (kpi.kpi === 'Eficiência' && kpi.evidenceAnalysis) {
                ...
                let validBottomTeams = new Set();
                if (kpi.kpi === 'OS Dia' && report.specialAnalysis.osDiaAnalysis) {
                    validBottomTeams = new Set(this.filterOsDiaEvidence(report.specialAnalysis.osDiaAnalysis).map((a) => a.team));
                }
                else if (kpi.kpi === 'Utilização' && report.specialAnalysis.utilizacaoAnalysis) {
                    validBottomTeams = new Set(report.specialAnalysis.utilizacaoAnalysis.filter((a) => a.flaggedOrders && a.flaggedOrders.length > 0).map((a) => a.team));
                }
                else if (kpi.kpi === 'TME IMP' && report.specialAnalysis.tmeImpAnalysis) {
                    validBottomTeams = new Set(this.filterTmeImpEvidence(report.specialAnalysis.tmeImpAnalysis).map((a) => a.team));
                }
                else if (kpi.kpi === '1º Login' && report.specialAnalysis.primeiroLoginAnalysis) {
                    validBottomTeams = new Set(this.filterLoginEvidence(report.specialAnalysis.primeiroLoginAnalysis).map((a) => a.team));
                }
                else if (kpi.kpi === '1º Desloc.' && report.specialAnalysis.primeiroDeslocAnalysis) {
                    validBottomTeams = new Set(this.filterDeslocEvidence(report.specialAnalysis.primeiroDeslocAnalysis).map((a) => a.team));
                }
                else if (kpi.kpi === 'Retorno Base' && report.specialAnalysis.retornoBaseAnalysis) {
                    validBottomTeams = new Set(this.filterRetornoEvidence(report.specialAnalysis.retornoBaseAnalysis).map((a) => a.team));
                }
                rawBottom = rawBottom.filter(t => validBottomTeams.has(t.team));
            }
        }
    ...
    analyticChartData(kpi) {
        return this.chartService.analyticChartData(kpi, this.reportData());
    }
    trackByFilterKey(_index, filter) {
        return filter.key;
    }
    trackByReportFilterKey(_index, filter) {
        return filter.key;
    }
    trackByOption(_index, option) {
        return option;
    }
    trackByTeam(_index, line) {
        return line.team;
    }
    trackByDayIndex(_index, pt) {
        return pt.dayIndex;
    }
    objectKeys(obj) {
        return Object.keys(obj);
    }
    calcIdleMin(analysis) {
        return analysis.tempPrepTotalMin + analysis.semOrdemTotalMin;
    }
    calcIdlePct(analysis) {
    ...
    eficienciaFlagLabel(flag) {
        return this.reportData()?.flagMeta?.labels[flag] ?? flag;
    }
    tmeImpFlagLabel(flag) {
        return this.reportData()?.flagMeta?.labels[flag] ?? flag;
    }
    loginFlagLabel(flag) {
        return this.reportData()?.flagMeta?.labels[flag] ?? flag;
    }
    deslocFlagLabel(flag) {
        if (flag === 'desloc_lento' || flag === 'desloc_muito_lento')
            return '1º Desloc. ≥25min';
        return this.reportData()?.flagMeta?.labels[flag] ?? flag;
    }
    retornoFlagLabel(flag) {
        return this.reportData()?.flagMeta?.labels[flag] ?? flag;
    }
    // ─── Alert body — delegates to pre-computed backend texts ────────────────
    nf(v, minDec = 1, maxDec = 1) {
        return v.toLocaleString('pt-BR', { minimumFractionDigits: minDec, maximumFractionDigits: maxDec });
    }
    osDiaAlertBody(flag, ev) {
        return ev.alertTexts?.[flag] ?? '';
    }
    semOsDetailText(d, nrOrdemDespachoAnterior) {
        // Always compute fresh (ignore pre-computed label/body) to ensure stats are always shown
        const SEM_OS_LIMIT = 10;
        const pctAbove = (min) => Math.round((min - SEM_OS_LIMIT) / SEM_OS_LIMIT * 100);
        const fmtAvg = (pct, avg) => {
            if (!Number.isFinite(pct) || !Number.isFinite(avg) || (avg ?? 0) <= 0)
                return '';
    ...
    alertSemOsDetails(details) {
        if (!details)
            return [];
        return details.filter((d) => d.type !== 'fim_jornada' || d.excess_min != null);
    }
    semOsDetailLabel(d, nrOrdemDespachoAnterior) {
        const text = this.semOsDetailText(d, nrOrdemDespachoAnterior);
        const sep = text.indexOf(': ');
        return sep > -1 ? text.slice(0, sep) : text;
    }
    semOsDetailBody(d, nrOrdemDespachoAnterior) {
    ...
    tempPrepAltoText(ev) {
        ...
        if (val == null || !Number.isFinite(val))
            return ev.alertTexts?.['temp_prep_alto'] ?? '';
        const limit = 10;
    ...
    triagemAltoText(ev) {
        ...
        if (val == null || !Number.isFinite(val))
            return ev.alertTexts?.['triagem_alto'] ?? '';
        const limit = 10;
        const pct = Math.round((val - limit) / limit * 100);
    ...
    eficienciaAlertBody(flag, ev) {
        return ev.alertTexts?.[flag] ?? '';
    }
    tmeImpAlertBody(flag, ev) {
        return ev.alertTexts?.[flag] ?? '';
    }
    loginAlertBody(flag, ev) {
        return ev.alertTexts?.[flag] ?? '';
    }
    deslocAlertBody(flag, ev) {
        return ev.alertTexts?.[flag] ?? '';
    }
    retornoAlertBody(flag, ev) {
        return ev.alertTexts?.[flag] ?? '';
    }
    // ─────────────────────────────────────────────────────────────────────────────────────────────────
    getFimJornadaDetail(ev) {
        return ev.sem_os_details?.find((d) => d.type === 'fim_jornada') ?? null;
    }
    /** Returns the body text for the 'antes_log_off_alto' orange warn item. */
    antesLogOffAltoText(ev) {
        const fj = this.getFimJornadaDetail(ev);
        if (!fj)
            return '';
        return this.semOsDetailBody(fj);
    }
    isOptionSelected(filter, option) {
        return filter.value.includes(option);
    }
    describeSelection(filter) {
    ...
    isReportOptionSelected(filter, option) {
        return filter.value.includes(option);
    }
    describeReportSelection(filter) {
        if (filter.value.includes(ALL_OPTION)) {
            return 'Todos';
        }
        if (filter.value.length === 0) {
    ...
    toggleReportFilterOption(key, option, multiSelect = false) {
    ...
    onReportSelectChange(key, event) {
        const value = String(event.target?.value ?? ALL_OPTION);
    ...
    beginOptionSelection(key, value, event) {
        ...
        const nextSelected = ctrlLike ? selected : new Set();
        if (shouldSelect) {
    ...
    continueOptionSelection(key, value, event) {
    ...
    dragScrollList(key, event) {
        ...
        const itemUnderCursor = document.elementFromPoint(event.clientX, mouseY);
        const button = itemUnderCursor?.closest('.option-item');
    ...
    applyDragRange(filter, currentValue) {
    ...
    applyFilterSelection(key, value) {
    ...
    syncSliderDOM() {
        ...
        if (this.sliderThumbMinRef?.nativeElement)
            this.sliderThumbMinRef.nativeElement.value = String(sMin);
        if (this.sliderThumbMaxRef?.nativeElement)
            this.sliderThumbMaxRef.nativeElement.value = String(sMax);
        // Update number inputs (single-month) with normalised resolved values
        if (!this.multiMonthSelected()) {
            const r = this.resolvedDayRange();
            if (this.minNumInputRef?.nativeElement)
                this.minNumInputRef.nativeElement.value = String(r.min);
            if (this.maxNumInputRef?.nativeElement)
                this.maxNumInputRef.nativeElement.value = String(r.max);
        }
        void total; // suppress unused-var lint
    }
    updateDayRangeSlider(boundary, event) {
        const absolute = Number(event.target?.value ?? Number.NaN);
    ...
    updateDayRangeFromInput(boundary, event) {
    ...
    updateDayRangeFromText(boundary, event) {
    ...
    cascadeReportFilters(filters, mode) {
        ...
        const filteredTeams = this.availableTeams.filter((team) => {
            ...
            if (!meta)
                return true; // Keep unknown teams if no filters applied? Actually, let's keep them if no filters.
            if (effectiveBases.length > 0 && !effectiveBases.includes(meta.base))
                return false;
            if (effectiveTipos.length > 0 && !effectiveTipos.includes(meta.type))
                return false;
            return true;
        });
    ...
    updateAvailableBasesForExport() {
        ...
        for (const team of this.availableTeams) {
            ...
            if (matchedBase) {
                if (matchedType === 'Própria')
                    proprias.add(matchedBase);
                else if (matchedType === 'Parceira')
                    parceiras.add(matchedBase);
            }
        }
        this.availablePropriasBases = Array.from(proprias);
        this.availableParceirasBases = Array.from(parceiras);
    }
    scheduleInstantReportRefresh() {
    ...
    updateReportDataAndDates(report) {
    ...
    monthOptionsFromSelection(selectedYear) {
    ...
    dayOptionsFromSelection(selectedYear, selectedMonth) {
     ...
     */
    dayLimitForMonthPosition(selectedYear, selectedMonth, position) {
    ...
    resolveValues(value, options, fallback) {
    ...
    orderSelection(options, selection) {
...
__decorate([
    ViewChild('sliderFill')
], DashboardComponent.prototype, "sliderFillRef", void 0);
__decorate([
    ViewChild('sliderThumbMin')
], DashboardComponent.prototype, "sliderThumbMinRef", void 0);
__decorate([
    ViewChild('sliderThumbMax')
], DashboardComponent.prototype, "sliderThumbMaxRef", void 0);
__decorate([
    ViewChild('minNumInput')
], DashboardComponent.prototype, "minNumInputRef", void 0);
__decorate([
    ViewChild('maxNumInput')
], DashboardComponent.prototype, "maxNumInputRef", void 0);
